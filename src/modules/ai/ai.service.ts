...
export const aiService = {
  async generate(prompt: string) {
    ...
    if (!generatedText) throw new Error("Failed to generate response");

    return { prompt, response: generatedText, model: "gpt-4o-mini-search" };
  },

  async generateProfile(prompt: string, token: string) {
    const currentProfile = await getProfileByToken(token);
    if (!currentProfile) throw Object.assign(new Error("Profile not found"), { statusCode: 404 });
...
Please provide detailed information about this person, including their full name, profession, achievements, and background. Focus on factual information from reliable sources.`;
    ...
    if (!detailsText) throw new Error("Failed to get information about the person");
...
[
  ...
  {"title": "Достижения", "description": "Основные достижения и успехи"}
    ...
    return { profile: currentProfile, createdBars, structuredBlocks, aiResponse: detailsText };
  },
};
